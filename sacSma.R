...
sacSma <- function(par, ini.states = c(0,0,0,0,0,0), prcp = precipit, pet = evapo,Q=Q_obs) {
  ...
  uztwc <- ini.states[1] # Upper zone tension water storage
  uzfwc <- ini.states[2] # Upper zone free water storage
  lztwc <- ini.states[3] # Lower zone tension water storage
  lzfsc <- ini.states[4] # Lower zone supplementary free water storage
  lzfpc <- ini.states[5] # Upper zone primary free water storage
  adimc <- ini.states[6] # Additional impervious area storage
  ...
  MAE <- mean(abs(Q[91:length(Q)]-simflow[91:length(Q)]))
  #return(list(tot = simflow, surf = surf_tot, base = base_tot))
  return(MAE)
}
