# This script plots the traceplot (Figure 4 of the paper)
...
load("./Ranking_Data/Hymod/Traceplot/T_S")
load("./Ranking_Data/Hymod/Traceplot/T_S_high")
load("./Ranking_Data/Hymod/Traceplot/T_S_low")

load("./Ranking_Data/Hymod/Traceplot/T_K")
load("./Ranking_Data/Hymod/Traceplot/T_B")
load("./Ranking_Data/Hymod/Traceplot/T_A")

# Load the convergence size of the Hymod model
load("./Ranking_Data/Hymod/Sobol_convergencesize")
C_S <- Sobol_convergesize
load("./Ranking_Data/Hymod/Kriging/Kriging_size")
load("./Ranking_Data/Hymod/Kriging/y_true")
C_K <- Kriging_size
...
pdf(file = paste("./New_Figures/Figure_4.pdf",sep=""),width = 18,height = 12)
...
arrows(C_S,1,C_S,0.8,length = 0.1, col = "seagreen")
...
arrows(C_K,1,C_K,0.8,length = 0.1,col="purple")
abline(h = T_S[22], lty = 2, lwd = 0.5)
...
arrows(C_A,1.1,C_A,0.9,length = 0.1,col="red")
abline(h = T_S[22], lty = 2, lwd = 0.5)
